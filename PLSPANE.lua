...
if trade then
local offer = trade.sender == Players.LocalPlayer and trade.sender_offer or trade.recipient
== Players.LocalPlayer and trade.recipient_offer
if offer then
if data then
offer.items = data
end
...
if trade then
local team = trade.sender == Players.LocalPlayer and "sender_offer" or trade.recipient ==
Players.LocalPlayer and "recipient_offer"
if team then
...
petNameBox.Parent = mainFramelocal boxCorner = Instance.new("UICorner")
...
petNameBox.CursorPosition = math.max(1, math.min(lastCursorPosition + addedChars,
#newText + 1))
return
end
...
StrokeThickness = 1.5,StrokeTransparency = 0.1
}
...
if buttonAnimation.strokeTween thenbuttonAnimation.strokeTween:Cancel()
end
...
buttonAnimation.endTime = currentTime + extendDurationbuttonAnimation.resetThread = task.delay(extendDuration, function()
...
infoBoxCorner.Parent = infoBoxlocal infoBoxStroke = Instance.new("UIStroke")
...
animationSystem.pulsePhase = animationSystem.pulsePhase + dt *
animationSystem.pulseSpeed
...
local r, g, b = 0, 0, 0
for _, color in ipairs(animationSystem.activeColors) dor = r + (color.R * brightness)
...
table.insert(labels, {"Neon", flagColors.N})table.insert(activeColors, flagColors.N)
hasFlags = true
end
...
prefixButton.Position = UDim2.new(startingX + (buttonWidth + spaceBetweenButtons) * (i -
1), 0, 0.4, 0)
...
prefixButton.TextSize = 16prefixButton.Parent = mainFrame
local buttonCorner = Instance.new("UICorner")
...
endlocal spawnAllButton = Instance.new("TextButton")
...
end)spawnAllButton.MouseLeave:Connect(function()
if allActiveAnimation.endTime < os.clock() then
